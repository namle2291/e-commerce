      ...
      <div className="absolute duration-300 z-10 opacity-0 group-hover:opacity-100 top-0 left-0 right-0 bottom-0 bg-white">
        <div className="flex flex-col">
          <div className="flex justify-between items-center p-[15px] border-b">
            <Link
              to={`/product/${data._id}`}
              className="max-w-[200px] line-clamp-1 hover:text-main_color"
            >
              {data.title}
            </Link>
            <span className="line-clamp-1">{formatPrice(data.price)} VND</span>
          </div>
          ...
          <div className="w-full duration-300">
            <SelectOption product={data} justify="center" />
          </div>
        </div>
      </div>
      <div className="p-[15px]">
        <div className="relative max-w-[345px] h-[345px] mb-[20px] overflow-hidden">
          <img
